...
#
# Derives the apex domain from custom_domain_name (e.g. api.example.com → zamait.in)
# and looks up the most-recent IMPORTED, ISSUED certificate for that domain.
# Covers both the apex and the wildcard SAN (*.example.com).
#
...
locals {
  ...
  # "api.example.com" → ["dev","zamait","in"] → join last 2 → "zamait.in"
