...
flowchart TD
    ...
    subgraph DNS["DNS — GoDaddy (example.com)"]
        CNAME["your-domain.example.com\nCNAME → &lt;api-gw-domain&gt;.execute-api.us-east-1.amazonaws.com"]
    end
    ...
    Client -->|"HTTPS GET your-domain.example.com/mock/api/users"| CNAME
...
| Step | Component | Detail |
|------|-----------|--------|
| 1 | Client | `GET https://your-domain.example.com/web/` |
...
aws elbv2 describe-load-balancers --region us-east-1 \
  --query 'LoadBalancers[?contains(DNSName,`&lt;nlb-id&gt;`)].{State:State.Code,Scheme:Scheme,Type:Type}'
```
...
```bash
# API
curl -sv https://your-domain.example.com/mock/api/users 2>&1 | grep "< HTTP"

# UI
curl -sv https://your-domain.example.com/web/ 2>&1 | grep "< HTTP"
```
