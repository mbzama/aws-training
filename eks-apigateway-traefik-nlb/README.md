...
After `terraform apply`, retrieve your endpoints:
...
| Service | URL pattern |
|---------|-------------|
| **UI** (Next.js) | `https://<custom_domain_name>/web` or the API Gateway URL from outputs |
| **API** (NestJS) | `https://<custom_domain_name>/mock` or the API Gateway URL from outputs |

> Set `custom_domain_name` and `acm_certificate_arn` in `terraform/terraform.tfvars` to enable a custom domain.
> Leave them empty to use the auto-generated API Gateway URL.
...
flowchart LR
    ...
    subgraph DNS["DNS  —  example.com"]
        DOMAIN["your-domain.example.com\nCNAME → API Gateway"]
    end

    subgraph EDGE["AWS Edge"]
        ACM["🔒 ACM\n*.example.com  TLS 1.2"]
        APIGW["API Gateway v2\nhrms-api-gateway\ncustom domain: your-domain.example.com"]
    end
...
flowchart TD
    ...
    %% ── Inbound: /mock flow ──────────────────────────────────────────────────
    INTERNET        -->|"HTTPS  your-domain.example.com/mock/api/..."| APIGW_EXT
    ...
    %% ── Inbound: /web flow ───────────────────────────────────────────────────
    APIGW_EXT       -->|"VPC Link  :80\nyour-domain.example.com/web/..."| VPCLink
...
|------|-----------|--------|
| 1 | Client | `GET https://your-domain.example.com/mock/api/users` |
...
| Step | Component | Detail |
|------|-----------|--------|
| 1 | Client | `GET https://your-domain.example.com/web/products` |
...
Phases:
1. **ACM certificate import** — prompts for `cert.pem`, `privkey.pem`, `chain.pem` for `*.example.com`; writes the ARN back to `terraform.tfvars`
...
Base URL: `https://your-domain.example.com/mock`
...
```bash
curl https://your-domain.example.com/mock/api/users
curl https://your-domain.example.com/mock/api/products
curl https://your-domain.example.com/mock/api/users/1
curl -X POST https://your-domain.example.com/mock/api/users \
...
Base URL: `https://your-domain.example.com/web`
...
env {
  name  = "API_URL"
  value = "https://your-domain.example.com/mock"
}
```
...
```bash
kubectl set env deployment/mock-web -n mock-web API_URL=https://your-domain.example.com/mock
...
| Wildcard ACM cert (`*.example.com`) | One imported certificate covers all subdomains |
...
# 7. End-to-end
curl -sv https://your-domain.example.com/mock/api/products 2>&1 | grep "< HTTP"
curl -sv https://your-domain.example.com/web/products       2>&1 | grep "< HTTP"
```
