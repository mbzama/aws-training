...
flowchart TB
        ...
        subgraph VPC["VPC: hrms-cluster-vpc  (10.0.0.0/16)"]
            ...
            subgraph AZ1["us-east-1a"]
                ...
                subgraph PrivSub1["Private Subnet  10.0.2.0/24"]
                    Node["Worker Node\nt3.medium · 20 GB EBS · hrms-ng-2"]
                    VPCCNI["vpc-cni  DaemonSet\nAssigns VPC IPs directly to pods\nEnables pod-to-pod routing in VPC"]
                    KProxy["kube-proxy  DaemonSet\nMaintains iptables rules\nfor Service → pod routing"]
                    CoreDNS["coredns  Deployment  (×2 replicas)\nCluster-internal DNS\nResolves Service names to ClusterIPs"]
                end
            end
    ...
    IGW        <-->|"EKS public API"| EKS
    NAT1        -->|"outbound internet"| PrivSub1
    NAT2        -->|"outbound internet"| PrivSub2
    ...
    Node        ---  KProxy
    Node        ---  CoreDNS
    VPCCNI      -->|"pod IPs allocated\nfrom VPC CIDR"| EKS
    CoreDNS    <-->|"DNS for Services + Pods"| Node
```
