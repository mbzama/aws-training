...
output "cluster_jdbc_url" {
  ...
  value       = "jdbc:redshift://${aws_redshift_cluster.main.endpoint}/${var.db_name}"
}
...
output "postgresql_connect_command" {
  description = "Connect to PostgreSQL from a host that has network access to the RDS instance"
...
output "postgresql_secret_arn" {
  ...
  value       = aws_secretsmanager_secret.pg_master.arn
}
