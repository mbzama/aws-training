...
variable "nlb_wait_seconds" {
  description = "Seconds to wait after Traefik is deployed before looking up the NLB DNS name"
  type        = number
  default     = 120
}

variable "acm_certificate_arn" {
  description = "ACM certificate ARN for NLB TLS termination on port 443"
  type        = string
  default     = ""
}
